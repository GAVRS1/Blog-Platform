...
#### Публичный доступ (туннель/удаленный доступ)

Чтобы сайт был доступен извне (например, через Cloudflare Tunnel), важно чтобы сервер слушал все сетевые интерфейсы:

```bash
dotnet run --project BlogContent.WebAPI/BlogContent.WebAPI --urls http://0.0.0.0:5000
```

Далее направьте ваш туннель на `http://localhost:5000`.
...
Dev‑сервер Vite должен слушать все интерфейсы. Это уже учтено в конфигурации, но при необходимости можно явно указать:

```bash
npm run dev -- --host 0.0.0.0 --port 5173
```

Далее направьте ваш туннель на `http://localhost:5173`.
