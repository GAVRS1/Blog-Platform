...
export default function FollowersPage() {
  ...
  async function load(p = 1) {
    setLoading(true);
    try {
      const res = await followsService.followers(userId, p, 20);
      setData(res);
      setPage(p);
    } catch (e) {
      toast.error(e.response?.data || 'Не удалось загрузить подписчиков');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      <div className="flex items-center justify-between mb-4">
        <h1 className="text-2xl font-bold">Подписчики #{userId}</h1>
      </div>

      {loading && <div className="flex justify-center py-10"><span className="loading loading-spinner text-primary"></span></div>}

      {!loading && data.items.length === 0 && (
        <div className="text-center opacity-60 py-10">Подписчиков пока нет</div>
      )}
